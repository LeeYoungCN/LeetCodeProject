...
#include "lc3xxx/lc30xx/lc306x/lc3068_find_the_maximum_sum_of_node_values.h"
...
protected:
    ...
    void RunTest(vector<int>& nums, int k, vector<vector<int>>& edges, long long expectResult);
};
...
void TEST_LC3068::RunTest(vector<int>& nums, int k, vector<vector<int>>& edges, long long expectResult)
{
    for (LC3068_FindTheMaximumSumOfNodeValues* inst : m_testList) {
        EXPECT_EQ(expectResult, inst->maximumValueSum(nums, k, edges)) << typeid(inst).name();
    }
}
